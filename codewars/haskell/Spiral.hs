...
spiralize n = [[ if elem (x,y) path then 1 else 0 | y <- [0..n-1]] | x <- [0..n-1]]
  where
    -- vectors are replicated to create a spiral path
    -- vectors ::= [right, down, left, up, ...]
    vecs = cycle [(0,1), (1,0), (0,-1), (-1,0)]
    -- repetitions from outside to the inside, starting with one full line,
    -- then repeating two lines of the same length,
